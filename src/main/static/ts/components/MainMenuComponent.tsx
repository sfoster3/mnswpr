...
> {
  ...
  clickStartNewGame(): void {
    const { width, height, mines } = this.state;
    this.setState({ loading: true }, () =>
      this.props.startNewGame({ width, height, count: mines })
    );
  }

  onChangeWidth({ target: { value } }: { target: { value: string } }): void {
    this.setState({ width: Number(value) });
  }

  onChangeHeight({ target: { value } }: { target: { value: string } }): void {
    this.setState({ height: Number(value) });
  }

  onChangeMines({ target: { value } }: { target: { value: string } }): void {
        ...
        <div className="form-row">
          <div className="col">
            ...
            <input
              ...
              placeholder="Mines"
              value={mines}
              onChange={this.onChangeMines.bind(this)}
              disabled={loading}
            />
          </div>
          <div className="col">
            <button
              className="form-control"
              disabled={loading}
              onClick={this.clickStartNewGame.bind(this)}
            >
              New Game
            </button>
          </div>
