...
trait MineFieldGenerator {
  ...
  protected def validateCount(width: Int, height: Int, count: Int): Unit = {
    if (count > ((width * height) / 2)) {
      throw new Exception(
        s"$count is too many mines for a board of ${width}x$height"
      )
    }
  }

  protected def getRandomSeed(seed: Option[Int]): Random = {
    val randSeed: Array[Byte] =
      seed.map(BigInt(_).toByteArray).getOrElse(SecureRandom.getSeed(100))
    new Random(BigInt(randSeed).toLong)
  }

  protected def doGeneration(
      start: Coordinate,
      width: Int,
      height: Int,
      count: Int,
