...
object GameSession {
  ...
  def props(
      ...
      generator: MineFieldGenerator = DefaultGenerator
  ): Props =
  ...
  private sealed case class Board(private val cells: Map[Coordinate, Cell]) {
    def updated(coordinate: Coordinate, cell: Cell): Board =
      Board(cells.updated(coordinate, cell))

    def removed(coordinate: Coordinate): Board =
      Board(cells.removed(coordinate))

    def apply(coordinate: Coordinate): Cell =
      cells.getOrElse(coordinate, UnknownCell())
  }
}
...
class GameSession(
  ...
  private def revealAndUpdate(
      mineField: MineField,
      board: Board,
      coordinates: Set[Coordinate]
  ): Unit = {
    val possibleCells: Map[Coordinate, Option[Cell]] = coordinates.map { c =>
      c ->
    ...
    val mines: Set[Coordinate] = possibleCells.collect {
      case (co: Coordinate, None) => co
    }.toSet
    if (mines.nonEmpty) {
      ...
      def _flood(cells: Map[Coordinate, Cell]): Map[Coordinate, Cell] = {
        val newCells: Map[Coordinate, Cell] = cells
          .collect {
          ...
          .map { co => co -> mineField.getNum(co) }
          .collect {
            case (co: Coordinate, Some(n)) if !cells.contains(co) =>
              co -> RevealedCell(n)
          }
          .toMap
        if (newCells.isEmpty) {
      ...
      val newCells: Map[Coordinate, Cell] = _flood(possibleCells.collect {
        case (co: Coordinate, Some(cell)) => co -> cell
      })
      val newBoard = newCells.foldLeft(board) {
        case (b: Board, (co: Coordinate, cell: Cell)) => b.updated(co, cell)
      }
  ...
  private def getVisibleBoard(
      board: Board,
      mines: Option[Set[Coordinate]] = None
  ): APIActionResponse = {
    ...
    val revealedCount = cells.values.count({
      case RevealedCell(_) => true
      case _               => false
    })
  ...
  private def addSolution(visibleBoard: VisibleBoard): APIActionResponse = {
    val toReveal =
      (visibleBoard.height * visibleBoard.width) - visibleBoard.cells.count(
        _._2 != UnknownCell()
      )
    val stats =
      if (toReveal < 20) BruteForceSolver.solve(visibleBoard)
      else SimpleSolver.solve(visibleBoard)
    APIActionResponse(visibleBoard.copy(stats = Some(stats.toSet)))
  }

  private def wrapReceive(pField: Option[MineField], board: Board): Receive = {
    case Flag(coordinate) =>
        ...
        case RevealedCell(n) if n != 0 =>
          ...
          if (adj.count(a => board(a) == FlaggedCell()) == n) {
            revealAndUpdate(
              mineField,
              board,
              adj.filter(a => board(a) == UnknownCell())
            )
          } else {
            sender() ! getVisibleBoard(board)
          }
        case RevealedCell(0) | UnknownCell() | FlaggedCell() =>
          sender() ! getVisibleBoard(board)
      }
    case GetVisible() => sender() ! getVisibleBoard(board)
    case Solve()      => sender() ! addSolution(getVisibleBoard(board).board)
  }
}
